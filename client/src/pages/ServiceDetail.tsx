...
export default function ServiceDetail() {
  ...
  // Estados para o Modal de Serviço Extra
  const [isExtraServiceModalOpen, setIsExtraServiceModalOpen] = useState(false);
  const [extraProcedure, setExtraProcedure] = useState("");
  ...
  // Função para criar Serviço Extra clonando os dados
  const handleCreateExtraService = async () => {
    if (!service) return;
    if (!extraProcedure.trim()) {
      return toast.error("Por favor, informe o nome do procedimento extra.");
    }
    ...
    try {
      await api.post("/trabalhos", {
        paciente_nome: service.patient,
        dentista_nome: service.dentist,
        procedimento: extraProcedure,
        valor_bruto: parseFloat(extraValue) || 0,
        prazo_entrega: service.prazo_entrega,
        prioridade: service.prioridade,
        forma_pagamento: service.forma_pagamento,
        status: "Pendente",
        descricao: `Serviço extra vinculado a partir da O.S. #${service.id}`
      });
      
      toast.success("Serviço extra criado e vinculado ao paciente!");
      setIsExtraServiceModalOpen(false);
      setExtraProcedure("");
      setExtraValue("");
    } catch (error) {
      toast.error("Erro ao criar serviço extra.");
    } finally {
  ...
  const triggerPdfPrint = () => {
    ...
    if (!printWindow) {
      return toast.error("Permita pop-ups para gerar o PDF.");
    }
    ...
    const htmlContent = `
      ...
      <body>
        ...
        <div class="info">
          <p><strong>Dentista/Parceiro:</strong> ${service.dentist.toUpperCase()} ${service.dentistPhone ? `&nbsp;|&nbsp; <strong>Tel:</strong> ${service.dentistPhone}` : ''}</p>
          <p><strong>Paciente:</strong> ${service.patient.toUpperCase()} ${service.patientPhone ? `&nbsp;|&nbsp; <strong>Tel:</strong> ${service.patientPhone}` : ''}</p>
          <p><strong>Serviço:</strong> #${String(service.id).padStart(5, '0')} &nbsp;&nbsp;&nbsp; <strong>Data de Entrada:</strong> ${service.createdAt || dataEmissao}</p>
          <p><strong>Prazo Previsto:</strong> ${prazoFormatado} &nbsp;&nbsp;&nbsp; <strong>Status Final:</strong> ${finalizadoFormatado}</p>
        </div>
        <table>
          <thead>
            <tr>
  ...
  return (
    <div className="min-h-screen bg-transparent p-6 relative">
      ...
      {isExtraServiceModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-2xl w-full max-w-md shadow-2xl relative">
            <button onClick={() => setIsExtraServiceModalOpen(false)} className="absolute top-4 right-4 p-1 text-neutral-500 hover:text-white transition-colors">
              <X className="w-5 h-5" />
            </button>
            <h3 className="text-xl font-black text-white uppercase tracking-tight mb-2 flex items-center gap-2">
              <Plus className="w-5 h-5 text-[#DEAE60]" /> Novo Serviço Extra
            </h3>
            <p className="text-sm text-neutral-400 mb-6">
              Será criada uma nova O.S. separada, já vinculada ao paciente <strong>{service.patient}</strong>.
            </p>
            <div className="space-y-4 mb-8">
              <div className="space-y-2">
                <Label className="text-xs font-bold text-neutral-400 uppercase">Novo Procedimento *</Label>
                <Input value={extraProcedure} onChange={e => setExtraProcedure(e.target.value)} className={inputBaseStyle} placeholder="Ex: Coroa Adicional" />
              </div>
              <div className="space-y-2">
                <Label className="text-xs font-bold text-neutral-400 uppercase">Valor (R$)</Label>
                <Input type="number" step="0.01" value={extraValue} onChange={e => setExtraValue(e.target.value)} className={inputBaseStyle} placeholder="0.00" />
              </div>
            </div>
            <div className="flex gap-3">
              <Button onClick={() => setIsExtraServiceModalOpen(false)} className="flex-1 bg-neutral-800 hover:bg-neutral-700 text-white">Cancelar</Button>
              <Button onClick={handleCreateExtraService} disabled={isSubmittingExtra} className="flex-1 bg-[#DEAE60] hover:bg-[#DEAE60]/90 text-neutral-950 font-black uppercase">
                {isSubmittingExtra ? "Criando..." : "Criar Serviço"}
              </Button>
            </div>
          </div>
        </div>
      ...
      {isExportModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-2xl w-full max-w-md shadow-2xl relative">
            <button onClick={() => setIsExportModalOpen(false)} className="absolute top-4 right-4 p-1 text-neutral-500 hover:text-white transition-colors">
              <X className="w-5 h-5" />
            </button>
            <h3 className="text-xl font-black text-white uppercase tracking-tight mb-2 flex items-center gap-2">
              <Download className="w-5 h-5 text-[#DEAE60]" /> Exportar Relatório
            </h3>
            <p className="text-sm text-neutral-400 mb-6">Quais custos deseja adicionar como <strong>"Custo Extra"</strong> no PDF?</p>
            <div className="space-y-3 mb-8 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
              {service.costs.map(cost => (
                <label key={cost.id} className="flex items-center gap-3 p-3 bg-neutral-950/50 border border-neutral-800/80 rounded-lg cursor-pointer hover:bg-neutral-800/50 transition-colors">
                  <input type="checkbox" className="w-5 h-5 rounded border-neutral-700 bg-neutral-900 text-[#DEAE60] focus:ring-[#DEAE60]" checked={selectedCostsPdf.includes(cost.id)} onChange={() => toggleCostForPdf(cost.id)} />
              ...
              {service.costs.length === 0 && (
                <div className="text-center py-6 bg-neutral-950/30 rounded-lg border border-neutral-800/50">
                  <p className="text-xs text-neutral-500 font-bold uppercase tracking-widest">Nenhum custo registrado.</p>
                </div>
              )}
            </div>
            <div className="flex gap-3">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-black text-white uppercase tracking-tight">{service.procedure}</h1>
            <p className="text-neutral-400 text-sm mt-1 sm:mt-2">Paciente: {service.patient} | Dentista: {service.dentist}</p>
          </div>
        </div>
        <div className="flex flex-wrap gap-3">
          <Button onClick={() => setIsExportModalOpen(true)} className="bg-neutral-800 hover:bg-neutral-700 text-white font-bold rounded-lg flex items-center gap-2">
          ...
          {!editing && (
            <>
              ...
              <Button onClick={() => setEditing(true)} className="bg-[#DEAE60] hover:bg-[#DEAE60]/90 text-neutral-950 font-bold rounded-lg">
                Editar Caso
              </Button>
              <Button onClick={handleDeleteService} className="bg-red-500/10 hover:bg-red-500/20 text-red-500 font-bold rounded-lg border border-red-500/20">
              ...
              <div className="space-y-2">
                ...
                {editing ? (
                  <Select value={service.prioridade} onValueChange={(value) => handleFieldChange("prioridade", value)}>
                    <SelectTrigger className="bg-neutral-900 border-neutral-800 text-white focus:ring-1 focus:ring-[#DEAE60]/50"><SelectValue placeholder="Selecione a prioridade" /></SelectTrigger>
                    <SelectContent className="bg-neutral-900 border-neutral-800">
        ...
        <div className="space-y-6">
          <Card className="bg-neutral-900 border-neutral-800 p-6 shadow-xl">
            ...
            {editing ? (
              <div className="space-y-4">
                ...
                >
                  <SelectTrigger className="bg-neutral-900 border-neutral-800 text-white focus:ring-1 focus:ring-[#DEAE60]/50"><SelectValue /></SelectTrigger>
                  <SelectContent className="bg-neutral-900 border-neutral-800">
              ...
              <div className="bg-neutral-950/50 p-4 rounded-lg border border-neutral-800/50 mb-2">
                ...
                {editing ? (
                  <Select value={service.forma_pagamento} onValueChange={(value) => handleFieldChange("forma_pagamento", value)}>
                    <SelectTrigger className="bg-neutral-900 border-neutral-800 text-white focus:ring-1 focus:ring-[#DEAE60]/50 h-10"><SelectValue placeholder="Selecione..." /></SelectTrigger>
                    <SelectContent className="bg-neutral-900 border-neutral-800">
                  ...
                  <div className="space-y-3">
                    {service.costs.map((cost) => (
                      <div key={cost.id} className="flex items-center gap-2">
                        ...
                        <Button type="button" onClick={() => handleRemoveCost(cost.id)} variant="ghost" className="h-9 w-9 p-0 text-neutral-500 hover:text-red-400 shrink-0"><Trash2 className="w-4 h-4" /></Button>
                      </div>
                    ))}
                  </div>
                ) : (
          ...
          {editing && (
            <div className="flex gap-4 sticky top-6">
              <Button onClick={() => { setEditing(false); window.location.reload(); }} className="flex-1 bg-neutral-800 hover:bg-neutral-700 text-white font-bold rounded-lg h-12">Cancelar</Button>
